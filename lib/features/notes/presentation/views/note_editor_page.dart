...
class _NoteEditorViewState extends State<_NoteEditorView> {
  ...
  void _togglePanel(ToolbarPanel panel) {
    setState(() {
      ...
      // 🌟 终极修复：绝不乱抢焦点！
      // 如果是点开标签面板，我们主动收起键盘让出空间。
      // 如果是点开 Aa 或 + 号，什么焦点操作都不做，绝不强制弹键盘！
              ...
              GestureDetector(
                ...
                onTap: () {
                  if (!viewModel.isReadOnly) {
                    ...
                    if (_activePanel != ToolbarPanel.none) {
  ...
  Widget _buildQuillEditor(ThemeData theme, NoteEditorViewModel viewModel) {
    final defaultTextStyle = TextStyle(
        fontSize: 16, height: 1.8, color: theme.colorScheme.onSurface.withValues(alpha: 0.9), letterSpacing: 0.4
    );
    final listTextStyle = TextStyle(
        fontSize: 16, height: 1.25, color: theme.colorScheme.onSurface.withValues(alpha: 0.9), letterSpacing: 0.4
    );
